/*
  ...
  1. New Tables
    - `kenig_rates`
      ...
      - `base` (text) - базовая валюта (BTC, ETH, USDT и т.д.)
      - `quote` (text) - котируемая валюта (RUB, USDT и т.д.)
      - `sell` (numeric) - курс продажи базовой валюты
      - `buy` (numeric) - курс покупки базовой валюты
      - `last_price` (numeric) - последняя цена сделки
...
-- Create the kenig_rates table if it doesn't exist
