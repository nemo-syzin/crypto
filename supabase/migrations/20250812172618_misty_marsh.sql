/*
  # Создание системы чата для KenigSwap
  ...
  2. Безопасность
    ...
    - Политики доступа для пользователей и операторов
...
CREATE POLICY "Участники могут видеть сообщения своих сессий"
  ON chat_messages
  FOR SELECT
  USING (
    session_id IN (
...
CREATE POLICY "Операторы могут видеть свои данные"
  ON chat_operators
  FOR SELECT
...
CREATE INDEX idx_chat_operators_user_id ON chat_operators(user_id);
CREATE INDEX idx_chat_operators_is_online ON chat_operators(is_online);
...
-- Вставляем тестового оператора (замените на реальные данные)
INSERT INTO chat_operators (user_id, name, email, is_online)
VALUES (
  gen_random_uuid(), -- Замените на реальный UUID пользователя-оператора
  'Анна Поддержка',
